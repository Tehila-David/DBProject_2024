...
#### קשר ראשון:
•	יישות TEACHER (מורה) מאגף חינוך יורשת מיישות Resident (תושב) מאגף גבייה.
•	ביישות TEACHER הסרנו את השדות FirstName, LastName.
•	אם תושב הוא מורה במוסד חינוכי יקבל הנחה (הוספת רשומה ביישות הנחה).

#### קשר שני:
•	נוסיף יישות הנקראת PRINCIPAL (מנהל) עם השדות הבאים: INSTITUTIONID, RESIDENT_ID, PRINCIPAL _id. 
